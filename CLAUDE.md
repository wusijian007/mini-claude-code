...
`myagent eval run` is the offline fixture-based agent regression suite ([packages/cli/src/eval.ts](packages/cli/src/eval.ts)): 7 inline `EvalTask`s drive the real `query()` loop with deterministic `FakeModel` scripts (read-only analysis, safe edit, read-only Bash, plan-mode-blocks-Write permission enforcement, explore sub-agent, proactive-compaction crossing the soft limit, and self-correction edit→verify-fail→fix→verify-pass via a scripted mock executor). Token/cost numbers are deterministic because `FakeModelStep`'s `assistant_message` accepts an optional scripted `usage`; the suite reports pass/turns/in/out/cache/cost per task plus totals under `.myagent/evals/runs/<runId>/`. It is also a CI gate via `packages/cli/test/eval.test.ts` (asserts all pass + pins the metric fingerprint) — a behavior regression in the agent loop flips it red.
...
8. Optional verification gate (M3.3, §4): with `QueryOptions.verify = { command, args, when, maxBounces }` set (CLI: `myagent agent --verify "<command>"`), the loop does NOT complete the moment the model stops calling tools. It first runs the verify command via `ToolContext.executor` (the M2.1 seam — NOT the whitelisted Bash tool, so arbitrary `npm test` / `tsc --noEmit` is allowed). Exit 0 → completes. Non-zero → the failure is injected as a reflective user turn (`reflectiveVerifyFailure`) and the loop continues an edit→test→fix cycle, bounded by `maxBounces` (default 2); exceeding it ends with a `verification_failed` terminal state instead of a silent `completed`. Each check yields a `verification` LoopEvent. Determinism: tests inject a mock `CommandExecutor` (the eval suite's `self-correction` task scripts exit codes `[1, 0]`).
